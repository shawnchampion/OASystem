...
<div align="center"> 
    ...
    <table width="780" border="0" cellspacing="0" cellpadding="0">
      <tr> 
        <td height="38">  <div align="center"> 全部工作列表<font size="3">
              ...
              <td bgcolor="#FFFFFF"><table width="100%" border="0" cellpadding="0" cellspacing="1" bgcolor="#999999">
                ...
                <table width="100%" border="0" cellpadding="2" cellspacing="1" bgcolor="#CCCCCC">
                  ...
                  <td><div align="center">${act.actid}</div></td>
                   <td><div align="left"><a href="<%=request.getContextPath()%>/SelectActByNo?actid=${act.actid}" target="_self">${act.actname}</a></div></td>
  ...
  第${cur}页 共${total}页 <a
							href="<%=request.getContextPath()%>/SelectWorkAll2?cur=1">首页</a>
							<c:if test="${cur!=1}">
								<a
									href="<%=request.getContextPath()%>/SelectWorkAll2?cur=${cur-1}">上一页</a>
							</c:if> <c:if test="${cur==1}">
	 	  上一页
	     </c:if> <c:if test="${cur!=total}">
								<a
									href="<%=request.getContextPath()%>/SelectWorkAll2?cur=${cur+1}"
									 >下一页</a>
							</c:if> <c:if test="${cur==total}">
	     下一页
	     </c:if> <a
							href="<%=request.getContextPath()%>/SelectWorkAll2?cur=${total}" >尾页</a>
   ...
   页 　<a href="workplan/workPlan.jsp" target="_self">添加工作安排</a></div>
   	...
   	<form action="<%=request.getContextPath()%>/SelectWorkAll2" name="myform">
